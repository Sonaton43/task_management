            ...
            <div class="card-body">
                <form class="row g-3 my-2" action="{{Route('user.update')}}" method="POST">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="email" class="form-control" value="{{$item->email}}" name="email" id="email" readonly>
                        <div class="text-danger">{{$errors->first('email')}}</div>
                    </div>

                    <div class="col-md-4">
                        <label for="phone" class="form-label">Phone<span class=" text-danger">*</span></label>
                        <input type="number" class="form-control" value="0{{$item->phone}}" name="phone" id="phone">
                        <div class="text-danger">{{$errors->first('phone')}}</div>
                    </div>

                    <div class="col-md-4">
                        <label for="designation" class="form-label">Designation</label>
                        <input type="text" class="form-control" value="{{$item->designation}}" name="designation" id="designation" readonly>
                        <div class="text-danger">{{$errors->first('designation')}}</div>
                    </div>

                    <div class="col-md-4">
                        <label for="role" class="form-label">Role<span class=" text-danger">*</span></label>
                        <input type="text" class="form-control" value="{{$item->role}}" name="role" id="role" readonly>
                    </div>
